...
function utf8_case_fold($text, $option = 'full')
{
	...
	if (!isset($uniarray['c']))
	{
		$uniarray['c'] = include(INCLUDE_PATH.'utf/data/case_fold_c.php');
	}

	// only set full if we need to
	if ($option === 'full' && !isset($uniarray['f']))
	{
		$uniarray['f'] = include(INCLUDE_PATH.'utf/data/case_fold_f.php');
	}
...
function utf8_clean_string($text)
{
	...
	if (empty($homographs))
	{
		$homographs = include(INCLUDE_PATH.'utf/data/confusables.php');
	}
