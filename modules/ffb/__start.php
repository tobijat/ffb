...
class __start extends FFB_Auth_User {
    ...
    public function __default() {
		...
		$connection = mysqli_connect($this->config->board_database_server, $this->config->board_database_name, $this->config->board_database_pw);
        $db = mysqli_select_db($connection, $this->config->board_database_name);
		...
		$num_entries = mysqli_num_rows($search_result);
		if($num_entries > 0) {
