	...
	<script type="text/javascript" src="<?echo FFB_BASE_PATH?>script/admin/match.js"></script>
</head>
<body onload="init();">
<div id="Container">
...
<form name="administration" action="./match" method="post" accept-charset="UTF-8">
    <div id="formline">
        ...
        <div id="forminput">
            <input type="hidden" name="match_round_post" value="<?echo $this->post['match_round'];?>">
    ...
    <div id="formline">
        ...
        <div id="forminput">
            <select name="match_date_day">
                <option value=""></option>
                <?for($i=1;$i<32;$i++) {?>
                    <option <?if($this->post['match_date_day'] == $i) echo 'selected';?> value="<?echo $i?>">
                        <?echo $i?>
                    </option>
                <?}?>
            </select>
            <select name="match_date_month">
                ...
                <?$months = array('January','February','March','April','May','June','July','August','September','October',
                                  'November','December');
                  for($i=0;$i<12;$i++) {?>
                    <option <?if($this->post['match_date_month'] == $i+1) echo 'selected';?> value="<?echo $i+1?>">
                        <?echo $months[$i]?>
                    </option>
                <?}?>
            </select>
            <select name="match_date_year">
                ...
                <?$now = date('Y',time());
                  for($i=$now+3;$i>$now-5;$i--) {?>
                    <option <?if($this->post['match_date_year'] == $i) echo 'selected';?> value="<?echo $i?>">
                        <?echo $i?>
                    </option>
                <?}?>
            </select>
    ...
    <div id="formline">
        <div id="formdescr">* Hometeam : <br>
            <input type="hidden" name="match_hometeam_id_post" value="<?echo $this->post['match_hometeam_id'];?>">
            <select name="match_hometeam_id">
                <option value="">Select Hometeam..</option>
            </select> :
        </div>
        <div id="forminput"> Guestteam *<br>
            <input type="hidden" name="match_guestteam_id_post" value="<?echo $this->post['match_guestteam_id'];?>">
    ...
    <div id="formline">
        <div id="formdescr">Goals Hometeam : <br>
            <select name="match_homescore">
                ...
                <!--
                <?for($i=0;$i<20;$i++) {?>
                    <option <?if(strval($this->post['match_homescore']) == strval($i)) echo 'selected';?> value="<?echo $i?>">
                        <?echo $i?>
                    </option>
                <?}?>
                //-->
        ...
        <div id="forminput"> Goals Guestteam<br>
            <select name="match_guestscore">
                ...
                <!--
                <?for($i=0;$i<20;$i++) {?>
                    <option <?if(strval($this->post['match_guestscore']) == strval($i)) echo 'selected';?> value="<?echo $i?>">
                        <?echo $i?>
                    </option>
                <?}?>
                //-->
    ...
    <div id="formline">
        <div id="formdescr">Penalties Hometeam : <br>
            <select name="match_homescore_penalty">
                ...
                <!--
                <?for($i=0;$i<20;$i++) {?>
                    <option <?if(strval($this->post['match_homescore_penalty']) == strval($i)) echo 'selected';?> value="<?echo $i?>">
                        <?echo $i?>
                    </option>
                <?}?>
                //-->
        ...
        <div id="forminput"> Penalties Guestteam<br>
            <select name="match_guestscore_penalty">
                ...
                <!--
                <?for($i=0;$i<20;$i++) {?>
                    <option <?if(strval($this->post['match_guestscore_penalty']) == strval($i)) echo 'selected';?> value="<?echo $i?>">
                        <?echo $i?>
                    </option>
                <?}?>
                //-->
    ...
    <div id="formline">
        ...
        <div id="forminput">
            <input type="text" class="input" name="match_status" value="<?echo $this->post['match_status'];?>">
        </div>
        <div id="formclear"></div>
    </div>
    ...
    <div id="formline">
        ...
        <input type="hidden" name="match_id" value="<?echo $this->post['match_id'];?>">
            ...
            <div id="listline">
                <div id="listdescr">
                    <div id="matchdate">
                        ...
                        ?>
                        <b><?echo $item['match_date'];?></b>
                    </div>
                    <div id="matchteams">
                    <?echo $item['match_hometeam_name'].' <img src="'.FFB_BASE_PATH.FFB_IMAGE_PATH.'flags/'.strtolower($item["match_hometeam_nationality"]).'.gif" width="20px" height="15px"> : <img src="'.FFB_BASE_PATH.FFB_IMAGE_PATH.'flags/'.strtolower($item["match_guestteam_nationality"]).'.gif" width="20px" height="15px"> '.$item['match_guestteam_name'];?>
                    </div>
                    <div id="matchround">
                        (<?echo $item['match_round_name'];?>)
                    </div>
            ...
            <div id="listline">
                <div id="listsymbol">
                    <form method="POST" action="./match">
                        <input type="hidden" name="match_id" value="<?echo $item['match_id']?>">
                        <input type="image" src="<?echo FFB_BASE_PATH.FFB_IMAGE_PATH?>symbols/edit.png" title="edit the entry" value="match_administration_change" name="match_administration_change">
                        <input type="image" src="<?echo FFB_BASE_PATH.FFB_IMAGE_PATH?>symbols/delete.png" title="delete the entry" value="match_administration_delete" name="match_administration_delete">
                    </form>
                </div>
            </div>
            <div id="listclear"></div>
