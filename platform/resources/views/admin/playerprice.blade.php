...
@section('content')
    ...
    <section class="panel admin-main" aria-labelledby="admin-playerprice-dynamic-title">
        ...
        <form class="admin-form" method="post" action="{{ route('admin.playerprice.setMatchroundPlayerPrices') }}" accept-charset="UTF-8">
            ...
            <div class="admin-field">
                ...
                <select id="pp_dyn_matchround" name="matchround_id" @disabled(! $hasGame)>
            ...
            <div class="admin-field">
                ...
                <select id="pp_dyn_margin" name="price_margin" @disabled(! $hasGame)>
            ...
            <div class="admin-actions">
                <button type="submit" class="admin-submit" name="set_playerprice_submit" value="1" @disabled(! $hasGame)>
                    Set Player Prices
                </button>
            </div>
    ...
    <section class="panel admin-main admin-playerprice-warn" aria-labelledby="admin-playerprice-elo-game-title">
        <div class="section-head">
            <h2 id="admin-playerprice-elo-game-title">ELO BasePrices for Game</h2>
        </div>
        <p class="hint">
        ...
        <form class="admin-form" method="post" action="{{ route('admin.playerprice.setGameEloTeamPrices') }}" accept-charset="UTF-8">
            @csrf
            <div class="admin-field">
                <label for="pp_game_max">Max Price</label>
                <select id="pp_game_max" name="max_price" @disabled(! $hasGame)>
            ...
            <div class="admin-field">
                ...
                <select id="pp_game_min" name="min_price" @disabled(! $hasGame)>
            ...
            <div class="admin-actions">
                <button type="submit" class="admin-submit" name="set_teamprice_for_game_submit" value="1" @disabled(! $hasGame)>
                    Set Team prices
                </button>
            </div>
    ...
    <section class="panel admin-main admin-playerprice-warn" aria-labelledby="admin-playerprice-elo-round-title">
        ...
        <form class="admin-form" method="post" action="{{ route('admin.playerprice.setMatchroundEloTeamPrices') }}" accept-charset="UTF-8">
            ...
            <div class="admin-field">
                ...
                <select id="pp_round_matchround" name="matchround_id" @disabled(! $hasGame)>
                    <option value="">Select Matchround..</option>
                    @foreach ($matchrounds as $round)
                        <option value="{{ $round['matchround_id'] }}" @selected((string) old('matchround_id') === (string) $round['matchround_id'])>
                            {{ $round['matchround_title'] }}
            ...
            <div class="admin-field">
                ...
                <select id="pp_round_max" name="max_price" @disabled(! $hasGame)>
                    <option value="">max price..</option>
                    @foreach ($priceOptions as $price)
                        <option value="{{ $price }}" @selected((string) old('max_price') === (string) $price)>
                            {{ $price }}
            ...
            <div class="admin-field">
                ...
                <select id="pp_round_min" name="min_price" @disabled(! $hasGame)>
                    <option value="">min price..</option>
                    @foreach ($priceOptions as $price)
                        <option value="{{ $price }}" @selected((string) old('min_price') === (string) $price)>
                            {{ $price }}
            ...
            <div class="admin-actions">
                <button type="submit" class="admin-submit" name="set_teamprice_for_matchround_submit" value="1" @disabled(! $hasGame)>
                    Set Team prices
                </button>
            </div>
            <p class="hint"><strong>(do only click once!)<br>(do not update during tournament!)</strong></p>
