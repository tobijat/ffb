...
abstract class BaseFfbUserscore extends BaseObject  implements Persistent
{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(FfbUserscorePeer::USERSCORE_ID) ) {
	...
	{
		$pos = FfbUserscorePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			case 0:
				...
				break;
			case 1:
				return $this->getUserscoreUserId();
				break;
			case 2:
				return $this->getUserscoreGameId();
				break;
			case 3:
				return $this->getUserscoreTotal();
				break;
			case 4:
				return $this->getUserscoreWcPoints();
				break;
			default:
				return null;
				break;
	...
	{
		$keys = FfbUserscorePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getUserscoreId(),
			$keys[1] => $this->getUserscoreUserId(),
			$keys[2] => $this->getUserscoreGameId(),
			$keys[3] => $this->getUserscoreTotal(),
			$keys[4] => $this->getUserscoreWcPoints(),
		);
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			case 0:
				$this->setUserscoreId($value);
				break;
			case 1:
				$this->setUserscoreUserId($value);
				break;
			case 2:
				$this->setUserscoreGameId($value);
				break;
			case 3:
				$this->setUserscoreTotal($value);
				break;
			case 4:
				$this->setUserscoreWcPoints($value);
				break;
		} // switch()
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setUserscoreId($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setUserscoreUserId($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setUserscoreGameId($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setUserscoreTotal($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setUserscoreWcPoints($arr[$keys[4]]);
	}
	...
	{
		return null === $this->getUserscoreId();
	}
	...
	{
		$this->userscore_id = null;
		$this->userscore_user_id = null;
		$this->userscore_game_id = null;
		$this->userscore_total = null;
		$this->userscore_wc_points = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
