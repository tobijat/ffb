...
abstract class BaseFfbPlayerstatsPeer {
	...
	public static function addSelectColumns(Criteria $criteria, $alias = null)
	{
		if (null === $alias) {
			...
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_ID');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_PLAYERTEAM_ID');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_MATCH_ID');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_MATCHROUND_ID');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_GOALS');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_ASSISTS');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_MINUTES');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_MINUTE_IN');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_MINUTE_OUT');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_CARDS');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_OWNGOALS');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_PENALTIESLOST');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_PENALTIESSAVED');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_PENALTYSHOOTOUT_SAVE');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_PENALTYSHOOTOUT_LOST');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_PENALTYSHOOTOUT_HIT');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE_GOALS');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE_ASSISTS');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE_MINUTES');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE_CARDS');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE_OWNGOALS');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE_PENALTIESLOST');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE_PENALTIESSAVED');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE_OPPGOALS');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE_NOOPPGOALS');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE_HIGH_LOSS');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE_HIGH_WIN');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE_PENALTYSHOOTOUT_SAVE');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE_PENALTYSHOOTOUT_LOST');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE_PENALTYSHOOTOUT_HIT');
			$criteria->addSelectColumn($alias . '.PLAYERSTATS_SCORE');
		}
	}
	 ...
	 * @throws     PropelException Any exceptions caught during processing will be
	 *		 rethrown wrapped into a PropelException.
	 * @return     array (FfbPlayerstats object, last column rank)
	 */
	public static function populateObject($row, $startcol = 0)
	{
		$key = FfbPlayerstatsPeer::getPrimaryKeyHashFromRow($row, $startcol);
		if (null !== ($obj = FfbPlayerstatsPeer::getInstanceFromPool($key))) {
			// We no longer rehydrate the object, since this can cause data loss.
			// See http://www.propelorm.org/ticket/509
			// $obj->hydrate($row, $startcol, true); // rehydrate
			$col = $startcol + FfbPlayerstatsPeer::NUM_COLUMNS;
		} else {
			$cls = FfbPlayerstatsPeer::OM_CLASS;
			$obj = new $cls();
	...
	public static function doUpdate($values, ?PropelPDO $con = null)
	{
		...
		if ($values instanceof Criteria) {
			...
			$value = $criteria->remove(FfbPlayerstatsPeer::PLAYERSTATS_ID);
			if ($value) {
				$selectCriteria->add(FfbPlayerstatsPeer::PLAYERSTATS_ID, $value, $comparison);
			} else {
				$selectCriteria->setPrimaryTableName(FfbPlayerstatsPeer::TABLE_NAME);
			}
	...
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$con->beginTransaction();
			$affectedRows += BasePeer::doDeleteAll(FfbPlayerstatsPeer::TABLE_NAME, $con, FfbPlayerstatsPeer::DATABASE_NAME);
	 ...
	 */
	 public static function doDelete($values, ?PropelPDO $con = null)
	 {
		...
		} elseif ($values instanceof FfbPlayerstats) { // it's a model object
