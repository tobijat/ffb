...
abstract class BaseFfbNoAds extends BaseObject  implements Persistent
{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(FfbNoAdsPeer::NO_ADS_ID) ) {
	...
	{
		$pos = FfbNoAdsPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			case 0:
				...
				break;
			case 1:
				return $this->getNoAdsUserIdIp();
				break;
			case 2:
				return $this->getNoAdsSlotId();
				break;
			default:
				return null;
				break;
	...
	{
		$keys = FfbNoAdsPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getNoAdsId(),
			$keys[1] => $this->getNoAdsUserIdIp(),
			$keys[2] => $this->getNoAdsSlotId(),
		);
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			case 0:
				$this->setNoAdsId($value);
				break;
			case 1:
				$this->setNoAdsUserIdIp($value);
				break;
			case 2:
				$this->setNoAdsSlotId($value);
				break;
		} // switch()
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setNoAdsId($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setNoAdsUserIdIp($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setNoAdsSlotId($arr[$keys[2]]);
	}
	...
	public function getFfbAdsSlot(PropelPDO $con = null)
	{
		if ($this->aFfbAdsSlot === null && ($this->no_ads_slot_id !== null)) {
			$this->aFfbAdsSlot = FfbAdsSlotQuery::create()->findPk($this->no_ads_slot_id, $con);
	...
	{
		$this->no_ads_id = null;
		$this->no_ads_user_id_ip = null;
		$this->no_ads_slot_id = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
