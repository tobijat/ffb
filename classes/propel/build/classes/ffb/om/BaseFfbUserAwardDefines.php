...
abstract class BaseFfbUserAwardDefines extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'FfbUserAwardDefinesPeer';
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				FfbUserAwardDefinesQuery::create()
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(FfbUserAwardDefinesPeer::USER_AWARD_DEFINES_ID) ) {
	...
	{
		$pos = FfbUserAwardDefinesPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			case 0:
				...
				break;
			case 1:
				return $this->getUserAwardDefinesAwardId();
				break;
			case 2:
				return $this->getUserAwardDefinesRank();
				break;
			case 3:
				return $this->getUserAwardDefinesRankName();
				break;
			case 4:
				return $this->getUserAwardDefinesAim();
				break;
			case 5:
				return $this->getUserAwardDefinesAimDbtable();
				break;
			case 6:
				return $this->getUserAwardDefinesAimOperator();
				break;
			case 7:
				return $this->getUserAwardDefinesAimCount();
				break;
			case 8:
				return $this->getUserAwardDefinesAimAutomatic();
				break;
			case 9:
				return $this->getUserAwardDefinesAimFunctionName();
				break;
			case 10:
	...
	{
		$keys = FfbUserAwardDefinesPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getUserAwardDefinesId(),
			$keys[1] => $this->getUserAwardDefinesAwardId(),
			$keys[2] => $this->getUserAwardDefinesRank(),
			$keys[3] => $this->getUserAwardDefinesRankName(),
			$keys[4] => $this->getUserAwardDefinesAim(),
			$keys[5] => $this->getUserAwardDefinesAimDbtable(),
			$keys[6] => $this->getUserAwardDefinesAimOperator(),
			$keys[7] => $this->getUserAwardDefinesAimCount(),
		...
		if ($includeForeignObjects) {
			if (null !== $this->aFfbUserAward) {
				$result['FfbUserAward'] = $this->aFfbUserAward->toArray($keyType, $includeLazyLoadColumns, true);
			}
		}
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			case 0:
				$this->setUserAwardDefinesId($value);
				break;
			case 1:
				$this->setUserAwardDefinesAwardId($value);
				break;
			case 2:
				$this->setUserAwardDefinesRank($value);
				break;
			case 3:
				$this->setUserAwardDefinesRankName($value);
				break;
			case 4:
				$this->setUserAwardDefinesAim($value);
				break;
			case 5:
				$this->setUserAwardDefinesAimDbtable($value);
				break;
			case 6:
				$this->setUserAwardDefinesAimOperator($value);
				break;
			case 7:
				$this->setUserAwardDefinesAimCount($value);
				break;
			case 8:
				$this->setUserAwardDefinesAimAutomatic($value);
				break;
			case 9:
				$this->setUserAwardDefinesAimFunctionName($value);
				break;
			case 10:
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setUserAwardDefinesId($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setUserAwardDefinesAwardId($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setUserAwardDefinesRank($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setUserAwardDefinesRankName($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setUserAwardDefinesAim($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setUserAwardDefinesAimDbtable($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setUserAwardDefinesAimOperator($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setUserAwardDefinesAimCount($arr[$keys[7]]);
	...
	{
		return null === $this->getUserAwardDefinesId();
	}
	 ...
	 * If this FfbUserAwardDefines is new, it will return
		...
		if(null === $this->collFfbUserAwardFinisheds || null !== $criteria) {
			if ($this->isNew() && null === $this->collFfbUserAwardFinisheds) {
				...
				$collFfbUserAwardFinisheds = FfbUserAwardFinishedQuery::create(null, $criteria)
					->filterByFfbUserAwardDefines($this)
					->find($con);
				if (null !== $criteria) {
	...
	{
		$this->user_award_defines_id = null;
		$this->user_award_defines_award_id = null;
		$this->user_award_defines_rank = null;
		$this->user_award_defines_rank_name = null;
		$this->user_award_defines_aim = null;
		$this->user_award_defines_aim_dbtable = null;
		$this->user_award_defines_aim_operator = null;
		$this->user_award_defines_aim_count = null;
