...
abstract class BaseFfbInvitation extends BaseObject  implements Persistent
{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(FfbInvitationPeer::INVITATION_ID) ) {
	...
	{
		$pos = FfbInvitationPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			case 0:
				...
				break;
			case 1:
				return $this->getInvitationSenderId();
				break;
			case 2:
	...
	{
		$keys = FfbInvitationPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getInvitationId(),
			$keys[1] => $this->getInvitationSenderId(),
			$keys[2] => $this->getInvitationEmail(),
			$keys[3] => $this->getInvitationDate(),
		);
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			case 0:
				...
				break;
			case 1:
				$this->setInvitationSenderId($value);
				break;
			case 2:
				$this->setInvitationEmail($value);
				break;
			case 3:
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setInvitationId($arr[$keys[0]]);
		if (array_key_exists($keys[1], $arr)) $this->setInvitationSenderId($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setInvitationEmail($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setInvitationDate($arr[$keys[3]]);
	}
	...
	{
		return null === $this->getInvitationId();
	}
	...
	public function clear()
	{
		...
		$this->invitation_sender_id = null;
