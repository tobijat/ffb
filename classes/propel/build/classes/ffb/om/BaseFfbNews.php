...
abstract class BaseFfbNews extends BaseObject  implements Persistent
{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(FfbNewsPeer::NEWS_ID) ) {
	...
	{
		$pos = FfbNewsPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 3:
				...
				break;
			case 4:
				return $this->getNewsPriority();
				break;
			case 5:
				return $this->getNewsGameId();
				break;
			case 6:
				return $this->getNewsSymbol();
				break;
			default:
				return null;
				break;
	...
	{
		$keys = FfbNewsPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getNewsId(),
			$keys[1] => $this->getNewsTitle(),
			$keys[2] => $this->getNewsText(),
			$keys[3] => $this->getNewsDate(),
			$keys[4] => $this->getNewsPriority(),
			$keys[5] => $this->getNewsGameId(),
			$keys[6] => $this->getNewsSymbol(),
		);
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 1:
				...
				break;
			case 2:
				$this->setNewsText($value);
				break;
			case 3:
				$this->setNewsDate($value);
				break;
			case 4:
				$this->setNewsPriority($value);
				break;
			case 5:
				$this->setNewsGameId($value);
				break;
			case 6:
				$this->setNewsSymbol($value);
				break;
		} // switch()
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setNewsTitle($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setNewsText($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setNewsDate($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setNewsPriority($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setNewsGameId($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setNewsSymbol($arr[$keys[6]]);
	}
	...
	public function clear()
	{
		...
		$this->news_text = null;
		$this->news_date = null;
		$this->news_priority = null;
		$this->news_game_id = null;
		$this->news_symbol = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
