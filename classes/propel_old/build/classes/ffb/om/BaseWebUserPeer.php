...
abstract class BaseWebUserPeer {
	...
	public static function addSelectColumns(Criteria $criteria, $alias = null)
	{
		if (null === $alias) {
			...
			$criteria->addSelectColumn($alias . '.USER_LIP');
			$criteria->addSelectColumn($alias . '.USER_DATE_REGISTER');
			$criteria->addSelectColumn($alias . '.USER_DATE_LLOGIN');
			$criteria->addSelectColumn($alias . '.USER_DATE_LACTION');
			$criteria->addSelectColumn($alias . '.USER_ACTIVATION_CODE');
			$criteria->addSelectColumn($alias . '.USER_MAILSERVICE');
		}
	}
	...
	public static function clearRelatedInstancePool()
	{
		...
		// Invalidate objects in WebLogPeer instance pool, 
		// since one or more of them may be deleted by ON DELETE CASCADE/SETNULL rule.
		...
		// Invalidate objects in WebAdminPeer instance pool, 
		// since one or more of them may be deleted by ON DELETE CASCADE/SETNULL rule.
	 ...
	 * @throws     PropelException Any exceptions caught during processing will be
	 *		 rethrown wrapped into a PropelException.
	 * @return     array (WebUser object, last column rank)
	 */
	public static function populateObject($row, $startcol = 0)
	{
		$key = WebUserPeer::getPrimaryKeyHashFromRow($row, $startcol);
		if (null !== ($obj = WebUserPeer::getInstanceFromPool($key))) {
			// We no longer rehydrate the object, since this can cause data loss.
			// See http://www.propelorm.org/ticket/509
			// $obj->hydrate($row, $startcol, true); // rehydrate
			$col = $startcol + WebUserPeer::NUM_COLUMNS;
		} else {
			$cls = WebUserPeer::OM_CLASS;
			$obj = new $cls();
	...
	public static function doUpdate($values, PropelPDO $con = null)
	{
		...
		if ($values instanceof Criteria) {
			...
			$value = $criteria->remove(WebUserPeer::USER_ID);
			if ($value) {
				$selectCriteria->add(WebUserPeer::USER_ID, $value, $comparison);
			} else {
				$selectCriteria->setPrimaryTableName(WebUserPeer::TABLE_NAME);
			}
	...
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$affectedRows += BasePeer::doDeleteAll(WebUserPeer::TABLE_NAME, $con, WebUserPeer::DATABASE_NAME);
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		} elseif ($values instanceof WebUser) { // it's a model object
		...
		try {
			...
			$c = clone $criteria;
			$affectedRows += WebUserPeer::doOnDeleteCascade($c, $con);
