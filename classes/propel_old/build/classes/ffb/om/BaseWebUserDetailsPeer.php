...
abstract class BaseWebUserDetailsPeer {
	...
	private static $fieldNames = array (
		...
		BasePeer::TYPE_RAW_COLNAME => array ('USER_ID', 'USER_DETAILS_AVATAR', 'USER_DETAILS_PHOTO', 'USER_DETAILS_WEBSITE', 'USER_DETAILS_ZIP', 'USER_DETAILS_STREET', 'USER_DETAILS_CITY', 'USER_DETAILS_PHONE', 'USER_DETAILS_FFB_FAVOURITE_TEAM', 'USER_DETAILS_FFB_OWN_TEAM', 'USER_DETAILS_FFB_OWN_PLAYER', 'USER_DETAILS_FFB_SELECTED_GAME', 'USER_DETAILS_LAST_UPDATE', ),
	...
	public static function addSelectColumns(Criteria $criteria, $alias = null)
	{
		if (null === $alias) {
			...
			$criteria->addSelectColumn($alias . '.USER_DETAILS_AVATAR');
			$criteria->addSelectColumn($alias . '.USER_DETAILS_PHOTO');
			$criteria->addSelectColumn($alias . '.USER_DETAILS_WEBSITE');
			$criteria->addSelectColumn($alias . '.USER_DETAILS_ZIP');
			$criteria->addSelectColumn($alias . '.USER_DETAILS_STREET');
			$criteria->addSelectColumn($alias . '.USER_DETAILS_CITY');
			$criteria->addSelectColumn($alias . '.USER_DETAILS_PHONE');
			$criteria->addSelectColumn($alias . '.USER_DETAILS_FFB_FAVOURITE_TEAM');
			$criteria->addSelectColumn($alias . '.USER_DETAILS_FFB_OWN_TEAM');
			$criteria->addSelectColumn($alias . '.USER_DETAILS_FFB_OWN_PLAYER');
			$criteria->addSelectColumn($alias . '.USER_DETAILS_FFB_SELECTED_GAME');
			$criteria->addSelectColumn($alias . '.USER_DETAILS_LAST_UPDATE');
		}
	}
	 ...
	 * @throws     PropelException Any exceptions caught during processing will be
	 *		 rethrown wrapped into a PropelException.
	 * @return     array (WebUserDetails object, last column rank)
	 */
	public static function populateObject($row, $startcol = 0)
	{
		$key = WebUserDetailsPeer::getPrimaryKeyHashFromRow($row, $startcol);
		if (null !== ($obj = WebUserDetailsPeer::getInstanceFromPool($key))) {
			// We no longer rehydrate the object, since this can cause data loss.
			// See http://www.propelorm.org/ticket/509
			// $obj->hydrate($row, $startcol, true); // rehydrate
			$col = $startcol + WebUserDetailsPeer::NUM_COLUMNS;
		} else {
			$cls = WebUserDetailsPeer::OM_CLASS;
			$obj = new $cls();
	...
	public static function doUpdate($values, PropelPDO $con = null)
	{
		...
		if ($values instanceof Criteria) {
			...
			$value = $criteria->remove(WebUserDetailsPeer::USER_ID);
			if ($value) {
				$selectCriteria->add(WebUserDetailsPeer::USER_ID, $value, $comparison);
			} else {
				$selectCriteria->setPrimaryTableName(WebUserDetailsPeer::TABLE_NAME);
			}
	...
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$con->beginTransaction();
			$affectedRows += BasePeer::doDeleteAll(WebUserDetailsPeer::TABLE_NAME, $con, WebUserDetailsPeer::DATABASE_NAME);
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		} elseif ($values instanceof WebUserDetails) { // it's a model object
