...
abstract class BaseFfbPollPeer {
	...
	private static $fieldNames = array (
		...
		BasePeer::TYPE_RAW_COLNAME => array ('POLL_ID', 'POLL_TITLE', 'POLL_START', 'POLL_END', 'POLL_GAME_ID', 'POLL_LOCATION', 'POLL_TYPE', 'POLL_VISIBLE', ),
	...
	private static $fieldKeys = array (
		...
		BasePeer::TYPE_RAW_COLNAME => array ('POLL_ID' => 0, 'POLL_TITLE' => 1, 'POLL_START' => 2, 'POLL_END' => 3, 'POLL_GAME_ID' => 4, 'POLL_LOCATION' => 5, 'POLL_TYPE' => 6, 'POLL_VISIBLE' => 7, ),
	...
	public static function addSelectColumns(Criteria $criteria, $alias = null)
	{
		if (null === $alias) {
			...
			$criteria->addSelectColumn($alias . '.POLL_TITLE');
			$criteria->addSelectColumn($alias . '.POLL_START');
			$criteria->addSelectColumn($alias . '.POLL_END');
			$criteria->addSelectColumn($alias . '.POLL_GAME_ID');
			$criteria->addSelectColumn($alias . '.POLL_LOCATION');
			$criteria->addSelectColumn($alias . '.POLL_TYPE');
			$criteria->addSelectColumn($alias . '.POLL_VISIBLE');
		}
	}
	 ...
	 * @throws     PropelException Any exceptions caught during processing will be
	 *		 rethrown wrapped into a PropelException.
	 * @return     array (FfbPoll object, last column rank)
	 */
	public static function populateObject($row, $startcol = 0)
	{
		$key = FfbPollPeer::getPrimaryKeyHashFromRow($row, $startcol);
		if (null !== ($obj = FfbPollPeer::getInstanceFromPool($key))) {
			// We no longer rehydrate the object, since this can cause data loss.
			// See http://www.propelorm.org/ticket/509
			// $obj->hydrate($row, $startcol, true); // rehydrate
			$col = $startcol + FfbPollPeer::NUM_COLUMNS;
		} else {
			$cls = FfbPollPeer::OM_CLASS;
			$obj = new $cls();
	...
	public static function doUpdate($values, PropelPDO $con = null)
	{
		...
		if ($values instanceof Criteria) {
			...
			$value = $criteria->remove(FfbPollPeer::POLL_ID);
			if ($value) {
				$selectCriteria->add(FfbPollPeer::POLL_ID, $value, $comparison);
			} else {
				$selectCriteria->setPrimaryTableName(FfbPollPeer::TABLE_NAME);
			}
	...
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$con->beginTransaction();
			$affectedRows += BasePeer::doDeleteAll(FfbPollPeer::TABLE_NAME, $con, FfbPollPeer::DATABASE_NAME);
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		} elseif ($values instanceof FfbPoll) { // it's a model object
