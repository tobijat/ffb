...
abstract class BaseFfbAds extends BaseObject  implements Persistent {
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = FfbAdsPeer::doInsert($this, $con);
	 ...
	 * Otherwise if this FfbAds has previously been saved, it will retrieve
	 * related FfbAdsAllocations from storage. If this FfbAds is new, it will return
		...
		if ($this->collFfbAdsAllocations === null) {
			if ($this->isNew()) {
			   $this->collFfbAdsAllocations = array();
				...
				$criteria->add(FfbAdsAllocationPeer::ADS_ALLOCATION_ADS_ID, $this->ads_id);
